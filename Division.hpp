...
namespace octo
{
    ...
    std::enable_if_t<std::is_convertible<T1, T2>::value, Division<T2>> operator/(const T1& lhs, Signal<T2>& rhs) { return {lhs, rhs}; }
    
    //! Combine a scalar and a signal into a division
    template <class T1, class T2>
    std::enable_if_t<std::is_convertible<T1, T2>::value, Division<T2>> operator/(const T1& lhs, Signal<T2>&& rhs) { return {lhs, std::move(rhs).moveToHeap()}; }
    
    //! Combine a scalar and a signal into a division
    template <class T1, class T2>
    std::enable_if_t<std::is_convertible<T2, T1>::value, Division<T1>> operator/(Signal<T1>& lhs, const T2& rhs) { return {lhs, rhs}; }
    ...
    //! Combine a scalar and a signal into a division
    template <class T1, class T2>
    std::enable_if_t<std::is_convertible<T2, T1>::value, Division<T1>> operator/(Signal<T1>&& lhs, const T2& rhs) { return {std::move(lhs).moveToHeap(), rhs}; }
