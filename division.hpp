...
namespace octo
{
    ...
    std::enable_if_t<std::is_convertible<T1, T2>::value, Division<T2>> operator/(const T1& lhs, Signal<T2>& rhs)
    {
    ...
    //! Combine a scalar and a signal into a division
    template <class T1, class T2>
    std::enable_if_t<std::is_convertible<T2, T1>::value, Division<T1>> operator/(Signal<T1>& lhs, const T2& rhs)
    {
    ...
    template <class T1, class T2>
    Division<std::common_type_t<T1, T2>> operator/(Signal<T1>& lhs, Signal<T2>& rhs)
    {
        if (lhs.getClock() != rhs.getClock())
            throw std::invalid_argument("cannot divide two signals that do not use the same clock");
